...
Forward Watch keeps the last published virtual AIS values when a target is no longer detected. This avoids sending null clears that some Signal K to NMEA converters cannot handle.

NMEA AIS export compatibility is disabled by default. When enabled, Forward Watch uses the MMSI embedded in each virtual vessel context and publishes AIS-like fields using the same object shapes produced by real NMEA0183 AIS input: `mmsi`, `name`, `navigation.position`, `navigation.speedOverGround = 0`, `sensors.ais.class = B`, `communication.callsignVhf`, `design.aisShipType`, `design.length`, `design.beam`, `sensors.ais.fromBow`, and `sensors.ais.fromCenter`. Static vessel fields such as `mmsi`, `name`, `communication`, `design`, and `sensors.ais` are published as a vessel-root subtree, matching how Signal K expects static AIS data. Box dimensions are first estimated from the detected box size, estimated distance, and calibrated camera FOV; AIS dimensions are then inferred from those box dimensions. Wide boxes are treated as a side view where box width approximates vessel length, while squarer boxes are treated as a bow/stern view where box width approximates beam. AIS export enforces a minimum static length and beam of 2 m so downstream AIS encoders do not emit zero-width targets. These generated virtual targets must not be forwarded to public AIS networks such as MarineTraffic or AIS Hub.
