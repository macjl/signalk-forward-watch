...
NMEA AIS export compatibility is disabled by default. When enabled, Forward Watch uses the MMSI embedded in each virtual vessel context and publishes `sensors.ais.class = B`, synthetic crossing `navigation.courseOverGroundTrue`, `navigation.headingTrue`, `navigation.speedOverGround = 0`, `communication.callsignVhf`, approximate static AIS fields under `design.aisShipType`, `design.length.overall`, `design.beam`, and AIS antenna offsets under `sensors.ais.fromBow`, `sensors.ais.toStern`, `sensors.ais.fromCenter`, and `sensors.ais.toPort` so converter plugins such as `signalk-vessels-to-ais` can emit NMEA AIS sentences and CPA/TCPA alerts. The synthetic course assumes the target is crossing own vessel's track: targets on port or centered use `bearing + 90°`, targets on starboard use `bearing - 90°`. Length is estimated from the detected box width, estimated distance, and the plugin's horizontal FOV assumption, then bounded by detection class. These are generated virtual targets with estimated characteristics: do not forward them to public AIS networks such as MarineTraffic or AIS Hub.
